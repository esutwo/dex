// Package msoidc implements logging in through Microsoft OpenID Connect standard
// https://docs.microsoft.com/en-us/openspecs/windows_protocols/ms-oidce/718379cf-8bc1-487e-962d-208aeb8e70ee
package msoidc
...
import (
	...
	"github.com/esutwo/go-oidc/v3/oidc"
	"golang.org/x/oauth2"
...
func (c *Config) Open(id string, logger log.Logger) (conn connector.Connector, err error) {
	...
	return &msoidcConnector{
		...
		verifier: provider.Verifier(
			&oidc.Config{ClientID: clientID, AdfsCompatibility: true},
		),
		logger:                    logger,
		cancel:                    cancel,
...
var (
	_ connector.CallbackConnector = (*msoidcConnector)(nil)
	_ connector.RefreshConnector  = (*msoidcConnector)(nil)
)

type msoidcConnector struct {
...
func (c *msoidcConnector) Close() error {
	c.cancel()
	return nil
}

func (c *msoidcConnector) LoginURL(s connector.Scopes, callbackURL, state string) (string, error) {
...
func (c *msoidcConnector) HandlePOST(s connector.Scopes, r *http.Request) (identity connector.Identity, err error) {
	code := r.PostFormValue("code")

	token, err := c.oauth2Config.Exchange(r.Context(), code)
	if err != nil {
		return identity, fmt.Errorf("msoidc: failed to get token: %v", err)
	}
...
func (c *msoidcConnector) HandleCallback(s connector.Scopes, r *http.Request) (identity connector.Identity, err error) {
	q := r.URL.Query()
...
func (c *msoidcConnector) Refresh(ctx context.Context, s connector.Scopes, identity connector.Identity) (connector.Identity, error) {
	cd := connectorData{}
	err := json.Unmarshal(identity.ConnectorData, &cd)
	if err != nil {
		return identity, fmt.Errorf("msoidc: failed to unmarshal connector data: %v", err)
	}
	...
	if err != nil {
		return identity, fmt.Errorf("msoidc: failed to get refresh token: %v", err)
	}

	return c.createIdentity(ctx, identity, token)
}

func (c *msoidcConnector) createIdentity(ctx context.Context, identity connector.Identity, token *oauth2.Token) (connector.Identity, error) {
	rawIDToken, ok := token.Extra("id_token").(string)
	if !ok {
		return identity, errors.New("msoidc: no id_token in token response")
	}
	...
	if err := idToken.Claims(&claims); err != nil {
		return identity, fmt.Errorf("msoidc: failed to decode claims: %v", err)
	}
	...
	if c.getUserInfo {
		...
		if err != nil {
			return identity, fmt.Errorf("msoidc: error loading userinfo: %v", err)
		}
		if err := userInfo.Claims(&claims); err != nil {
			return identity, fmt.Errorf("msoidc: failed to decode userinfo claims: %v", err)
		}
	}
	...
	if len(c.hostedDomains) > 0 {
		...
		if !found {
			return identity, fmt.Errorf("msoidc: unexpected hd claim %v", hostedDomain)
		}
	}
	...
	if err != nil {
		return identity, fmt.Errorf("msoidc: failed to encode connector data: %v", err)
	}
	...
	if c.userIDKey != "" {
		...
		if !found {
			return identity, fmt.Errorf("msoidc: not found %v claim", c.userIDKey)
		}
